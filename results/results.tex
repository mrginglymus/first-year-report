...
Primers for standard cloning with multiple restriction sites were designed (see section~\ref{sec:plaspri}); however, after repeated cloning attempts we failed to obtain a final construct, and after several weeks this assembly method was abandoned.
...
\caption[PCR results for Gibson Assembly]{PCR to create overlaps for Gibson Assembly. Desired bands circled in red.  The first band is from the plasmid in Figure~\subref{fig:results:gibson:A206K}, between primers WAC13 and 14. The second and third bands are from the plasmid in Figure~\subref{fig:results:gibson:49}, between primers WAC7 and 12, and between WAC8 and 11 respectively. The fourth and fifth bands are from the plasmid in Figure~\subref{fig:results:gibson:289}, between primers WAC9 and 12, and between WAC10 and 11 respectively.

From the bands in lanes 1, 2 and 3 we can create pWAC1 with Gibson Assembly, and pWAC4 from lanes 1, 4 and 5.}
...
\caption[Image processing results on Nikon microscope]{Histograms of CheZ cluster fraction (fraction of CheZ protein in cell localised in the polar cluster). Solid red line marks mean average, dashed lines \(\pm\sigma\). \(p\) value \(=4.4\%\) using the Student's t-test. Microscope: Nikon Ti-E widefield (demo).}
...
\caption[Image processing results on Zeiss confocal microscope]{Histograms of CheZ cluster fraction (fraction of CheZ protein in cell localised in the polar cluster). Solid red line marks mean average, dashed lines \(\pm\sigma\). \(p\) value \(=0.0349\%\) using the Student's t-test. Microscope: Zeiss confocal.}
...
The first, in Figure~\ref{fig:microfluidics:simple}, is a simple Y shaped splitter with only one syringe active at any time. One syringe would be filled with a plain buffer, and the other with buffer with attractant/repellent at a set concentration. This allows for rapid switching on and off of the attractant/repellent.
